...
- Evaluate Meta's open-source [below](https://github.com/facebookincubator/below) (Apache 2.0, Rust) as a complementary historical resource recorder. The current monitoring solution is a predictive memory growth detector — it uses linear regression to identify growth trends, attribute them to specific processes/containers, and alert before OOM. `below` solves the adjacent problem: continuous recording of system-wide state (CPU, memory, I/O, pressure stall information, full cgroup2 hierarchy).